...
def select_squared_region(
    ...
    """ Selects a squared subregion of dataset with dims (lon, lat, others).
    ...
    ds = ds.sortby('lon')
    ds = ds.sortby("lat")
    selected_region = ds.sel(
        lon=slice(lons[0], lons[1]),  # Select longitudes in [min(lons), max(lons)]
        lat=slice(lats[0], lats[1])  # Select latitudes in [min(lats), max(lats)]
    )
...
def classify_percentiles(
    ...
    else: # Use the given percentiles as the quantile to classify
        dim_p_index = np.arange(percentile.size) # 
        quantile = xr.DataArray(
            data = percentile,
            dims = [dim_p,],
            coords = dict(
                dim_p = ([dim_p,], dim_p_index), # Note: Assumes that dim_p is given as an index (0,1,2,...)
            ),
        )
    ...
    classified_data = xr.where(
        ...
        x=1,
        y=np.nan, # Ensures that NaN values are kept if they were in original dataset.
    )
    classified_data = xr.where(
...
def linear_detrend(
    ...
    # Initialize empty datasets to store detrended data, trends, and p-values
    trend_da = xr.DataArray(
            data = np.empty((len(dataset[group_dim]), len(dataset[dim]))), 
            dims = [group_dim, dim],
            coords = {
                group_dim: dataset[group_dim].values, 
                dim: dataset[dim].values,
    ...
    p_value_da = xr.DataArray(
            data = np.empty((len(dataset[group_dim]))), 
            dims = [group_dim],
            coords = {
                group_dim: dataset[group_dim].values, 
                },
    ...
    for group in dataset[group_dim].values:
        ...
        # Perform linear regression to get the trend and p-value
        t = np.arange(len(group_data[dim]))
        slope, intercept, r_value, p_value, std_err = stats.linregress(t, group_data.values)

        # Compute the trend
        trend = slope * t + intercept
        
        # Subtract trend to get detrended data
        detrended_data = group_data - trend
        
        # Store results
        detrend_da.loc[{group_dim: group}] = detrended_data
        trend_da.loc[{group_dim: group}] = trend
        p_value_da.loc[{group_dim: group}] = p_value

    # Create new DataArray for trend, detrended data, and significance
    dataset["trend"] = trend_da
    dataset["detrended"] = detrend_da
    dataset["p_value"] = p_value_da
...
#     Description: 
#         Applies a bandpass-filter on the data for each group of the group_dim. 
#         Until now only works for 2-Dim xr.Datasets, because the data is transformed to pandas.dataframe.
#     Parameters:
#         data : Dataset that contains the data to apply the filter on
#         group_dim : The name of the dimension along which the data should be grouped
#         dv_name : The name of the data array with the values of interest for applying the filter
#         p_high : Upper threshold of the period to cut out (must be in the same timesteps as fs)
#         p_low : Lower threshold of the period to cut out (must be in the same timesteps as fs)
...
#     # Function to apply the bandpass filter to the data
#     #---
...
#     # Switch data from xarray to pandas dataframe
#     #---
#     data_ = data.copy().to_dataframe().dropna(how="all")
    
#     # Filter the Signal
#     #---
#     signal = data_.groupby(group_dim)[dv_name].apply(
#         lambda x: butter_bandpass_filter(x, 
#                                          lowcut=lowcut, 
#                                          highcut=highcut, 
#                                          fs=fs, 
#                                          order=order,
...
#     # Remove the signal from data and
#     # Transform back to Xarray
#     #---
#     removed_signal = data_.copy()
#     removed_signal["bandpass_signal"] = np.concatenate(signal)
#     removed_signal["bandpass_filtered"] = removed_signal[dv_name] - removed_signal["bandpass_signal"]
#     removed_signal = removed_signal.to_xarray()
...
#         dataset : Data for which the classification is done. 
#         var_name : The datavariable of the dataset which should be classified
#         percentile : Used to compute the quantile for classifying the data (is_compute=True) 
#         or used as the quantile itself (is_compute=False).
#         dim : The dimension of the data along which to calculate the percentile
#         skipna : Whether to skip NaN values when calculating the percentile. Should be set True in most cases.
#         dropna : Whether to drop NaN values at the beginning of the classification.
...
#                 skipna=skipna,
#             )
#     else: # Use the given percentile as the quantile to classify
#         quantile = percentile
#     # Create the filter for classification
#     above_percentile = data_ >= quantile
#     below_percentile = data_ < quantile
